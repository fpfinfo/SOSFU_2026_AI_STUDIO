-- Migration: add_coordinates_to_dcomarcas
-- Adiciona latitude e longitude à tabela dcomarcas para suportar visualização geográfica precisa
...
-- Comentários
COMMENT ON COLUMN public.dcomarcas.latitude IS 'Latitude do fórum da comarca (decimal, ex: -1.44754)';
COMMENT ON COLUMN public.dcomarcas.longitude IS 'Longitude do fórum da comarca (decimal, ex: -48.49245)';

-- Popula coordenadas reais das comarcas do Pará (fonte: IBGE / Google Maps)
UPDATE public.dcomarcas SET latitude = CASE comarca
...
CREATE INDEX IF NOT EXISTS idx_dcomarcas_coords ON public.dcomarcas (latitude, longitude) WHERE latitude IS NOT NULL;
