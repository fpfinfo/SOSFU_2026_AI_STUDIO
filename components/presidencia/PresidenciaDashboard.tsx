...
import {
    Award, FileText, CheckCircle2, Search, DollarSign,
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4"
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="text-center mb-6">
                    ...
                    <h2 className="text-xl font-black text-slate-800">Confirmar Homologação</h2>
                ...
                <div className="flex gap-3">
                    ...
                    <button onClick={handleSubmit} disabled={!isFilled || isProcessing}
                        ...
                        {isProcessing ? <Loader2 size={16} className="animate-spin" /> : <Award size={16} />}
                        {isProcessing ? 'Homologando...' : 'Homologar'}
                    </button>
                </div>
            </div>
        </div>
...
// ==================== MAIN COMPONENT ====================
export const PresidenciaDashboard: React.FC<PresidenciaDashboardProps> = ({ onNavigate, darkMode = false, userProfile }) => {
    const isGestor = userProfile?.dperfil?.slug === 'PRESIDENCIA_GESTOR';
    ...
    const MODULE_NAME = 'PRESIDENCIA';
    const WAITING_STATUS = 'WAITING_PRESIDENCIA_SIGNATURE'; 
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (user) {
                ...
                setUserName(profile?.full_name || 'Presidente');
            }

             const { data: solicitations } = await supabase.from('solicitations').select('*')
                .eq('status', WAITING_STATUS) // Presidencia Work queue
    ...
    const handleBatchSign = async (enteredPin: string) => {
         setProcessing(true);
        try {
             alert('Simulação: Lote PRESIDENCIA processado com sucesso');
             setSelectedTaskIds(new Set());
             fetchData();
        } catch (err) {
            console.error(err);
    ...
    const handleRejectDocument = async (task: SigningTask) => {
         if (!rejectReason.trim()) { alert('Informe o motivo.'); return; }
          setProcessing(true);
        try {
            alert(`Simulação: Processo PRESIDENCIA devolvido. Motivo: ${rejectReason}`);
            setRejectingId(null);
            setRejectReason('');
            fetchData();
        } catch (err) {
    ...
    return (
        <div className="max-w-[1400px] mx-auto px-6 py-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-white rounded-2xl border-2 border-slate-100 overflow-hidden">
                ...
                <div className="flex items-center gap-1 px-4 pt-4 pb-0">
                    <div className="flex items-center gap-1 p-1 bg-slate-100 rounded-xl">
                        {([
                            { id: 'inbox' as QueueTab, label: 'Pendentes de Homologação', icon: <Inbox size={14} />, count: signingTasks.length },
                            { id: 'minha_fila' as QueueTab, label: 'Gabinete', icon: <Users size={14} /> },
                            { id: 'processados' as QueueTab, label: 'Homologados', icon: <CheckCircle2 size={14} />, count: signedTasks.length },
                        ]).map(tab => (
            ...
            {/* ===== HISTORY ===== */}
            <div>
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Histórico Presidência</h3>
