        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4"
            ...
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 animate-in fade-in zoom-in-95 duration-200">
                <div className="text-center mb-6">
                    ...
                    <h2 className="text-xl font-black text-slate-800">Confirmar Tramitação</h2>
                        ...
                        {pin.map((digit, i) => (
                            <input key={i} ref={el => { inputsRef.current[i] = el; }}
                                ...
                                className={`w-14 h-16 text-center text-2xl font-black border-2 rounded-xl
                                    focus:border-sky-500 focus:ring-2 focus:ring-sky-100 outline-none transition-all
    ...
    return (
        <div className={`flex items-center gap-4 px-4 py-3.5 border-b border-slate-100 hover:bg-slate-50/70 transition-all group ${
            ...
            <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <Tooltip content="Tramitar" position="top">
                    <button onClick={onSign} className="p-1.5 hover:bg-sky-50 rounded-lg transition-colors">
    ...
    return (
        <div className="max-w-[1400px] mx-auto px-6 py-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex gap-4">
                 <button onClick={() => alert('Novo Processo SODPA - Modal')} className="px-4 py-2 bg-sky-600 text-white rounded-lg font-bold shadow-sm hover:bg-sky-700 transition-colors flex items-center gap-2">
                    <FileText size={18} /> Novo Processo
                </button>
                 <button onClick={() => alert('Consultar Tabela - Modal')} className="px-4 py-2 bg-indigo-600 text-white rounded-lg font-bold shadow-sm hover:bg-indigo-700 transition-colors flex items-center gap-2">
                    <Search size={18} /> Tabela Diárias
                </button>
            </div>
            ...
            {/* ===== HISTORY ===== */}
            <div>
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Histórico SODPA</h3>
