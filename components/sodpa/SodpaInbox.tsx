...
import { Search, Inbox, Users, CheckCircle2, FileText, CheckSquare, Loader2, ArrowRight, UserPlus, Sparkles, Plane, Receipt } from 'lucide-react';
import { supabase } from '../../lib/supabase';
import { AssignModal } from '../AssignModal';

interface SodpaInboxProps {
...
interface InboxItem {
    ...
    details?: string; // For destination/etc
}

export const SodpaInbox: React.FC<SodpaInboxProps> = ({ onNavigate, userProfile }) => {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            // 1. Fetch Solicitations (SODPA specific statuses)
            // Assuming WAITING_SODPA_ANALYSIS is the main inbox status
            const { data: sols } = await supabase
                ...
                // Filter for SODPA types if necessary, but status usually segregates? 
                // We might need to filter by unit or other field if multiple modules share status? 
                // Assuming status is robust enough for now or use client filter if needed.
                .order('created_at', { ascending: false })
                .limit(100);

            // 2. Fetch Accountabilities
            // Using generic accountabilities table but we need to identify SODPA ones.
            // For now, we fetch all potential candidates and client-filter if possible, 
            // or assume RLS handles it.
            // Statuses for SODPA could be 'PENDING', 'UNDER_ANALYSIS'. 
            // NOTE: TeamTable used WAITING_SOSFU for SOSFU. 
            // If SODPA uses same table, it might use WAITING_SODPA?
            const { data: pcs } = await supabase
                .from('accountabilities')
                .select(`*, analyst:analyst_id(full_name, avatar_url), profiles:requester_id(full_name)`)
                .or('status.eq.PENDING,status.eq.UNDER_ANALYSIS,status.eq.APPROVED,status.eq.REJECTED') 
                .order('created_at', { ascending: false })
                .limit(100);
            ...
            if (sols) {
                sols.forEach(s => {
                    // Simple check if it belongs to SODPA logic (e.g. Diárias)
                    // If no explicit type, rely on status.
                    if (s.status.includes('SODPA') || s.status === 'PAID') {
                         combined.push({
                            id: s.id,
                            type: 'SOLICITATION',
                            ...
                            analyst: s.analyst,
                            details: s.unit // e.g. Destination or Origin
                        });
                    }
                });
            }
            ...
            if (pcs) {
                pcs.forEach(p => {
                    // Check if accountability pertains to SODPA?
                    // Ideally we check linked solicitation type.
                    combined.push({
                        id: p.id,
                        type: 'ACCOUNTABILITY',
                        process_number: p.process_number,
                        beneficiary: p.profiles?.full_name || 'Desconhecido',
                        value: p.value || 0, // value might be amount_spent or grant
            ...
            const newItems = combined.filter(i => 
                (i.type === 'SOLICITATION' && ['WAITING_SODPA_ANALYSIS'].includes(i.status)) ||
                (i.type === 'ACCOUNTABILITY' && ['PENDING'].includes(i.status))
            );
            
            const analysisItems = combined.filter(i => 
                (currentUserId && i.analyst_id === currentUserId) ||
                (i.type === 'SOLICITATION' && ['WAITING_SODPA_EXECUTION', 'WAITING_SEFIN_SIGNATURE'].includes(i.status)) ||
                (i.type === 'ACCOUNTABILITY' && ['UNDER_ANALYSIS'].includes(i.status))
            );

            const doneItems = combined.filter(i => 
                ['PAID', 'APPROVED', 'REJECTED'].includes(i.status)
            );
    ...
    useEffect(() => {
        fetchData();
        const channel = supabase.channel('sodpa-inbox')
    ...
    const getFilteredItems = () => {
        ...
        switch (activeTab) {
            ...
            case 'ANALYSIS':
                list = items.filter(i => 
                    (currentUserId && i.analyst_id === currentUserId) ||
                    (i.type === 'SOLICITATION' && ['WAITING_SODPA_EXECUTION', 'WAITING_SEFIN_SIGNATURE'].includes(i.status)) || // Removed status check for WAITING_SODPA_ANALYSIS to avoid duplication? or logic needs to match counts
                    (i.type === 'ACCOUNTABILITY' && ['UNDER_ANALYSIS'].includes(i.status))
                );
                // Exclude items that are strictly new (unassigned) from 'Analysis' if we want separation?
                // SosfuInbox logic: Analysis tab = Assigned to Me OR Waiting Sefin/Correction.
                // Here: Assigned to Me OR Execution/Sefin.
                break;
            case 'DONE':
    ...
    const handleView = (item: InboxItem) => {
        if (item.type === 'SOLICITATION') {
            ...
            if (item.solicitation_id) {
                onNavigate('process_accountability', item.solicitation_id, item.id || undefined); // process_accountability usually takes soc_id?
                // Or handle based on how routing works for accountability detail
            }
        }
    };
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 px-4 pt-4 pb-0 bg-white">
                ...
                <div className="relative w-full md:w-auto">
                    ...
                    <input 
                        ...
                        className="pl-9 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm w-full md:w-64 focus:outline-none focus:ring-2 focus:ring-sky-200 focus:border-sky-300 transition-all" 
                    />
                </div>
            </div>

            {/* Context Bar */}
            <div className="px-6 py-2 bg-sky-50/50 border-b border-sky-100/50 flex items-center gap-2 mt-4">
                <Sparkles size={12} className="text-sky-400" />
                <span className="text-xs font-medium text-sky-700">
                    {activeTab === 'NEW' ? 'Diárias e passagens aguardando análise.' : 
                            ...
                            <tr>
                                <td colSpan={6} className="px-6 py-16 text-center">
                                    <div className="flex flex-col items-center justify-center">
                                        ...
                                        <p className="text-xs text-slate-400 mt-1">Tudo em dia na SODPA.</p>
                                    </div>
                                </td>
                            </tr>
                        ) : (
                                ...
                                >
                                    <td className="px-6 py-3.5">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                                                item.type === 'SOLICITATION' ? 'bg-sky-100 text-sky-600' : 'bg-amber-100 text-amber-600'
                                            }`}>
                                                {item.type === 'SOLICITATION' ? <Plane size={16} /> : <Receipt size={16} />}
                                            </div>
                                            <div>
                                    ...
                                    <td className="px-6 py-3.5">
                                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold uppercase
                                            ${item.status.includes('WAITING') || item.status === 'PENDING' ? 'bg-amber-100 text-amber-700' : ''}
                                            ${item.status === 'APPROVED' || item.status === 'PAID' ? 'bg-emerald-100 text-emerald-700' : ''}
                                            ${item.status === 'UNDER_ANALYSIS' ? 'bg-blue-100 text-blue-700' : ''}
                                            ${item.status === 'REJECTED' ? 'bg-red-100 text-red-700' : ''}
                                        `}>
                                            {item.status.replace(/WAITING_|SODPA_|SEFIN_/g, '').replace(/_/g, ' ')}
                                        </span>
                                    </td>
                                    <td className="px-6 py-3.5">
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); setSelectedItem(item); setIsAssignModalOpen(true); }}
                                            className={`flex items-center gap-2 px-2 py-1 rounded-full border text-[10px] transition-all bg-white hover:border-sky-300 ${item.analyst ? 'text-slate-700 border-slate-200' : 'text-slate-400 border-dashed border-slate-300'}`}
                                        >
                                            {item.analyst ? (
                                                <>
                                                    <div className="w-4 h-4 rounded-full bg-sky-100 text-sky-600 flex items-center justify-center text-[9px] font-bold">
                                    ...
                                    <td className="px-6 py-3.5 text-right">
                                        <button onClick={(e) => { e.stopPropagation(); handleView(item); }} className="p-1.5 text-sky-600 hover:bg-sky-100 rounded-lg transition-colors">
                                            <ArrowRight size={16} />
                                        </button>
                                    </td>
                                </tr>
