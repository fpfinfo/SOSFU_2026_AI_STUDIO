...
interface SodpaExpenseElementsSettingsProps {
    darkMode?: boolean;
}

export const SodpaExpenseElementsSettings: React.FC<SodpaExpenseElementsSettingsProps> = ({ darkMode = false }) => {
    const { elements, loading, refresh } = useExpenseElements();
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            setIsAdding(false);
            setEditingId(null);
            setForm({ codigo: '', descricao: '', categoria: 'Custeio', ativo: true });
            refresh();
        } catch (e) {
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className={`text-sm mt-1 ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                        Gerencie os códigos orçamentários disponíveis para lançamentos de despesa.
                    </p>
                </div>
                {!isAdding && (
                    <button
            ...
            {isAdding && (
                <div className={`p-6 rounded-2xl border-2 border-blue-100 shadow-xl ${darkMode ? 'bg-slate-800/50' : 'bg-blue-50/30'} animate-in slide-in-from-top-4`}>
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="font-bold text-blue-700">Adicionar Novo Código</h3>
                ...
                <table className="w-full text-sm text-left">
                    <thead className={darkMode ? 'bg-slate-700/50' : 'bg-slate-50'}>
                        <tr>
                            ...
                            <th className="px-6 py-4 font-black uppercase tracking-wider text-[10px] text-slate-500">Categoria</th>
                        ...
                        {loading ? (
                            ...
                            <tr>
                                <td colSpan={5} className="px-6 py-12 text-center">
                                    ...
                                    <p className="text-slate-400">Nenhum elemento encontrado.</p>
                                </td>
                            </tr>
                        ) : (
                            filteredElements.map(el => (
                                <tr key={el.id} className={`${darkMode ? 'hover:bg-slate-800/50' : 'hover:bg-slate-50/50'} transition-colors`}>
                                    <td className="px-6 py-4">
                                        <button 
                                            onClick={() => toggleStatus(el.id, el.ativo)}
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`text-[10px] px-2 py-0.5 rounded-md font-bold ${
                                            el.categoria === 'Capital' ? 'bg-teal-50 text-teal-600' : 'bg-slate-100 text-slate-600'
                                        }`}>
                                            {el.categoria}
                                        </span>
                                    </td>
