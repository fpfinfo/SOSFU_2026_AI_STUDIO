  ...
  return (
    <div className="w-full min-h-screen p-6 md:p-10 flex flex-col lg:flex-row gap-8 animate-in fade-in duration-500 bg-gray-50/30">
        ...
        {activeSection === 'Usuarios' && (
           <div className="p-10 space-y-10 animate-in slide-in-from-right-8">
              ...
              <div className="bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-sm">
                 <div className="space-y-4">
                    {profiles.filter(p => p.systemRole?.includes(module.toUpperCase())).map(p => (
                       <div key={p.id} className="flex items-center justify-between p-4 hover:bg-slate-50 rounded-2xl transition-all border-b border-slate-50 last:border-0 font-bold">
                          ...
                          <button onClick={() => updateProfile({ id: p.id, systemRole: 'USER' }).then(loadSectionData)} className="p-2 text-slate-300 hover:text-red-500 transition-colors"><i className="fa-solid fa-trash-can"></i></button>
                       </div>
                    ))}
                 </div>
              </div>
      ...
      {showTeamModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-6">
           <div className="bg-white w-full max-w-lg rounded-[3rem] shadow-2xl p-10 space-y-8">
              <h3 className="text-2xl font-black text-slate-900 tracking-tight uppercase tracking-tight">Vincular Membro Técnica</h3>
              <div className="space-y-4">
                 <select value={newTeamMember.userId} onChange={e => setNewTeamMember({...newTeamMember, userId: e.target.value})} className="w-full bg-slate-50 border p-4 rounded-xl font-bold uppercase text-[10px]"><option value="">Selecione o Servidor</option>{profiles.map(p => <option key={p.id} value={p.id}>{p.fullName}</option>)}</select>
                 <select value={newTeamMember.role} onChange={e => setNewTeamMember({...newTeamMember, role: e.target.value})} className="w-full bg-slate-50 border p-4 rounded-xl font-bold uppercase text-[10px]"><option value="">Selecione a Função</option><option value={`GESTOR_${module.toUpperCase()}`}>GESTOR {module.toUpperCase()}</option><option value={`ANALISTA_${module.toUpperCase()}`}>ANALISTA {module.toUpperCase()}</option></select>
              </div>
              <div className="flex gap-4">
