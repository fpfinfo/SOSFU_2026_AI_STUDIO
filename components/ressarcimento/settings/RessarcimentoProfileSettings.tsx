...
import { Shield, Settings, User, Search, RefreshCw, Plus, X, Loader2, AlertCircle, ChevronDown, Check, UserCircle2, FileText, FileBadge, DollarSign, Scale } from 'lucide-react';
...
const ROLE_STYLES: Record<string, { label: string; color: string; ring: string }> = {
  ...
  'RESSARCIMENTO': { label: 'Ressarcimento', color: 'text-teal-700', ring: 'ring-teal-200' },
...
export const RessarcimentoProfileSettings: React.FC = () => {
    ...
    const fetchAvailableRoles = async () => {
        const { data } = await supabase.from('dperfil').select('*').order('name');
    ...
    const fetchProfiles = async () => {
        setLoading(true);
        try {
            const { data, error } = await supabase.from('profiles').select('*, dperfil:perfil_id(id, slug, name, description)').order('full_name');
    ...
    const filteredProfiles = profiles.filter(profile => {
        ...
        const matchesRole = selectedRoleFilter === 'ALL' || currentRole.startsWith(selectedRoleFilter); // Changed to startsWith to handle groups
        ...
        <div className="animate-in fade-in duration-300 relative font-sans">
             <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                    <div className="flex items-center gap-3">
                        <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                            Gestão de Perfis (RBAC) - Visão Global
                        </h3>
                        <button 
                ...
                <button 
                    ...
                    className="flex items-center gap-2 bg-emerald-600 text-white px-5 py-2 rounded-lg text-sm font-bold hover:bg-emerald-700 transition-colors shadow-sm"
                >
                    <Plus size={16} />
                    Conceder Papel
                </button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-3 mb-6">
                {[
                    ...
                    { group: 'RESSARCIMENTO', label: 'Ressarcimento', icon: FileText },
                    ...
                    return (
                        <button 
                            ...
                            className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all
                                ${isActive ? 'bg-emerald-50 border-emerald-200 shadow-sm' : 'bg-white border-gray-100 hover:border-emerald-100 hover:shadow-sm'}
                            `}
                        >
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center mb-1.5 transition-colors ${isActive ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-400 group-hover:bg-emerald-50 group-hover:text-emerald-500'}`}>
                                <item.icon size={16} />
                            </div>
                            <span className={`text-[10px] font-bold uppercase tracking-wider ${isActive ? 'text-emerald-700' : 'text-gray-500'}`}>{item.label}</span>
                             <span className={`text-[10px] font-bold bg-gray-100 px-1.5 rounded-full mt-1 ${isActive ? 'bg-emerald-100 text-emerald-700' : 'text-gray-400'}`}>{count}</span>
                        </button>
                    );
                })}
            </div>
            ...
            <div className="space-y-2">
                {loading ? (
                    <div className="p-12 flex justify-center items-center text-gray-500 gap-2 bg-white rounded-xl border border-gray-100 animate-pulse">
                        <Loader2 size={20} className="animate-spin text-emerald-600" /> Carregando usuários...
                    </div>
                    ...
                    filteredProfiles.map((user) => {
                        ...
                        return (
                            <div key={user.id} className="group bg-white border border-gray-100 rounded-lg p-3 flex items-center justify-between hover:border-emerald-200 hover:shadow-sm transition-all">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-full bg-emerald-600 flex items-center justify-center text-white font-bold text-xs shadow-sm flex-shrink-0">
                                ...
                                <button onClick={() => handleOpenModal(user)} className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-[10px] font-bold uppercase tracking-wide transition-colors bg-white border-gray-200 text-gray-500 hover:border-emerald-300 hover:text-emerald-600`}>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
                        ...
                        <div className="p-6 space-y-6">
                            {/* Search and Form logic same as above but with Emerald theme */}
                                ...
                                {modalSearchQuery.length >= 3 && !selectedUser && (
                                    <div className="absolute z-10 w-[calc(100%-3rem)] mt-1 bg-white border border-gray-200 rounded-xl shadow-xl max-h-48 overflow-y-auto">
                                        {modalSearchResults.map(user => (
                                            <button key={user.id} onClick={() => { setSelectedUser(user); setModalSearchQuery(''); setModalSearchResults([]); if (user.perfil_id) setSelectedRoleId(user.perfil_id); }} className="w-full text-left p-3 flex items-center gap-3 hover:bg-emerald-50 transition-colors border-b border-gray-50 last:border-0">
                            ...
                            {selectedUser && (
                                <div className="flex items-center justify-between p-3 bg-emerald-50 border border-emerald-100 rounded-lg animate-in fade-in slide-in-from-top-2">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-emerald-600 font-bold border border-emerald-100">{getInitials(selectedUser.full_name)}</div>
                                    ...
                                    <button onClick={() => { setSelectedUser(null); setModalSearchQuery(''); setSelectedRoleId(''); }} className="text-emerald-400 hover:text-emerald-600 p-1"><X size={16} /></button>
                                </div>
                            )}
                            <div className="space-y-2">
                                <label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Papel a Conceder</label>
                                <div className="relative">
                                    <select value={selectedRoleId} onChange={(e) => setSelectedRoleId(e.target.value)} disabled={!selectedUser} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 appearance-none cursor-pointer disabled:bg-gray-50 disabled:text-gray-400 font-medium text-gray-700">
                        ...
                        <div className="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
                            ...
                            <button onClick={handleSaveRole} disabled={saving || !selectedUser || !selectedRoleId} className={`px-6 py-2 text-sm font-bold text-white bg-emerald-600 rounded-lg shadow-sm hover:bg-emerald-700 transition-all flex items-center gap-2 ${(saving || !selectedUser || !selectedRoleId) ? 'opacity-70 cursor-not-allowed' : ''}`}>
