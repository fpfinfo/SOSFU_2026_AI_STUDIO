            ...
            {items.map((item, idx) => (
              <div key={item.id} className="flex gap-4">
                <select 
                  ...
                  onChange={e => setItems(items.map(i => i.id === item.id ? {...i, classification: e.target.value} : i))} 
                  className="flex-1 px-6 py-4 bg-slate-50 rounded-2xl font-bold"
                >
                  <option value="">Selecione a Rubrica...</option>
                  {expenseElements.map(el => <option key={el.id} value={el.description}>{el.description}</option>)}
                </select>
                <input 
                  type="number" 
                  value={item.value} 
