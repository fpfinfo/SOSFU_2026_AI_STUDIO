...
// ==================== MAIN COMPONENT ====================
export const SeadDashboard: React.FC<SeadDashboardProps> = ({ onNavigate, darkMode = false }) => {
    ...
    const MODULE_NAME = 'SEAD';
    const WAITING_STATUS = 'WAITING_SEAD_ANALYSIS'; 
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            if (user) {
                ...
                setUserName(profile?.full_name || 'Servidor SEAD');
            }

             const { data: solicitations } = await supabase.from('solicitations').select('*')
                .eq('status', WAITING_STATUS) // SEAD Work queue
    ...
    const handleBatchSign = async (enteredPin: string) => {
         setProcessing(true);
        try {
             alert('Simulação: Lote SEAD processado com sucesso');
             setSelectedTaskIds(new Set());
             fetchData();
        } catch (err) {
            console.error(err);
    ...
    const handleRejectDocument = async (task: SigningTask) => {
         if (!rejectReason.trim()) { alert('Informe o motivo.'); return; }
          setProcessing(true);
        try {
            alert(`Simulação: Processo SEAD devolvido. Motivo: ${rejectReason}`);
            setRejectingId(null);
            setRejectReason('');
            fetchData();
        } catch (err) {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center h-96 gap-4">
                ...
                <p className="text-slate-500 font-medium">Carregando painel SEAD...</p>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-[1400px] mx-auto px-6 py-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-gradient-to-r from-emerald-800 via-emerald-700 to-teal-700 rounded-2xl p-6 shadow-lg">
                <h2 className="text-xl font-black text-white">Painel SEAD</h2>
                <p className="text-sm text-emerald-100 mt-1.5">
                    Olá, <span className="font-bold text-white">{userName}</span>. Gerencie aqui os processos administrativos da SEAD.
                </p>
            </div>
            ...
            <div className="flex gap-4">
                 <button onClick={() => alert('Novo Documento SEAD - Modal')} className="px-4 py-2 bg-emerald-600 text-white rounded-lg font-bold shadow-sm hover:bg-emerald-700 transition-colors flex items-center gap-2">
                    <FileText size={18} /> Novo Doc
                </button>
                 <button onClick={() => alert('Tramitar SEAD - Modal')} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-bold shadow-sm hover:bg-blue-700 transition-colors flex items-center gap-2">
                ...
                <div className="mt-2 max-h-[500px] overflow-y-auto">
                    {filteredTasks.length > 0 ? (
                        ...
                        <div className="text-center py-16">
                            ...
                            <p className="text-sm text-slate-400 mt-1">Tudo em dia para a SEAD.</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            {/* ===== HISTORY ===== */}
            <div>
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Histórico SEAD</h3>
