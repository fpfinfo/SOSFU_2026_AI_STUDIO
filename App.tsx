...
import SefinWorkstation from './components/SefinWorkstation';
import SgpWorkstation from './components/SgpWorkstation';
import PresidenciaWorkstation from './components/PresidenciaWorkstation';
...
const App: React.FC = () => {
  ...
  const fetchProfile = async (userId: string) => {
    try {
      ...
      if (data) {
        ...
        } else if (role.includes('SEFIN') || role.includes('ORDENADOR')) {
  ...
  const renderContent = () => {
    ...
    // Perfil do Usuário — Rota Universal (funciona em TODOS os módulos)
    if (activeTab === 'mais') {
      return <ProfilePage onForceRefresh={() => fetchProfile(session.user.id)} />;
    }
    ...
    if (activeModule !== 'usuarios') {
      ...
      // SEFIN tem seu próprio workstation dedicado
      if (activeModule === 'sefin') {
        switch (activeTab) {
          case 'gestao_config': return <ManagementSettings module={activeModule} />;
          case 'gestao_inbox': return <SefinWorkstation module={activeModule} onSelectRequest={setSelectedRequest} profile={profile} activeView="minutas" />;
          case 'gestao_contas': return <SefinWorkstation module={activeModule} onSelectRequest={setSelectedRequest} profile={profile} activeView="autorizados" />;
          case 'inicio': return <SefinWorkstation module={activeModule} onSelectRequest={setSelectedRequest} profile={profile} activeView="inicio" />;
          default: return <SefinWorkstation module={activeModule} onSelectRequest={setSelectedRequest} profile={profile} activeView="minutas" />;
        }
      }

      // SGP (SEGEP) tem seu próprio workstation dedicado
      if (activeModule === 'sgp') {
        switch (activeTab) {
          case 'gestao_config': return <ManagementSettings module={activeModule} />;
          case 'gestao_inbox': return <SgpWorkstation module={activeModule} onSelectRequest={setSelectedRequest} profile={profile} activeView="processos" />;
          case 'gestao_contas': return <SgpWorkstation module={activeModule} onSelectRequest={setSelectedRequest} profile={profile} activeView="autorizados" />;
          case 'inicio': return <SgpWorkstation module={activeModule} onSelectRequest={setSelectedRequest} profile={profile} activeView="inicio" />;
          default: return <SgpWorkstation module={activeModule} onSelectRequest={setSelectedRequest} profile={profile} activeView="processos" />;
        }
      }

      // Presidência (Gabinete) tem seu próprio workstation dedicado
      if (activeModule === 'presidencia') {
        switch (activeTab) {
          case 'gestao_config': return <ManagementSettings module={activeModule} />;
          case 'gestao_inbox': return <PresidenciaWorkstation module={activeModule} onSelectRequest={setSelectedRequest} profile={profile} activeView="despachos" />;
          case 'gestao_contas': return <PresidenciaWorkstation module={activeModule} onSelectRequest={setSelectedRequest} profile={profile} activeView="autorizados" />;
          case 'inicio': return <PresidenciaWorkstation module={activeModule} onSelectRequest={setSelectedRequest} profile={profile} activeView="inicio" />;
          default: return <PresidenciaWorkstation module={activeModule} onSelectRequest={setSelectedRequest} profile={profile} activeView="despachos" />;
        }
      }
